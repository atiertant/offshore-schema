...
JoinTables.prototype.buildJoins = function(collection) {
  ...
  if (Object.keys(collectionAttributes).length === 0)
    return [];
  ...
  collectionAttributes.forEach(function(attribute) {
    ...
    if (table)
      tables.push(self.buildTable(table));
  });
...
JoinTables.prototype.mapCollections = function(attributes) {
  ...
  for (var attribute in attributes) {
    if (! hop(attributes[attribute], 'collection'))
      continue;
...
JoinTables.prototype.parseAttribute = function(collectionName, attribute) {
  ...
  var tableAttributes = {};
  tableAttributes[collectionName] = {
  ...
  var associations = [];
  if (hop(attr, 'polymorphic')) {
    attr.collection.forEach(function(collection, index) {
      // Normalize `collection` property name to lowercased version
      attr.collection[index] = collection.toLowerCase();
      associations.push(attr.collection[index]);
    });
  } else {
    // Normalize `collection` property name to lowercased version
    attr.collection = attr.collection.toLowerCase();
    associations.push(attr.collection);
  }

  associations.forEach(function(collection) {
    // Grab the associated collection and ensure it exists
    var child = self.collections[collection];
    if (! child) {
      error = 'Collection ' + collectionName + ' has an attribute named ' + attribute.key + ' that is ' +
        'pointing to a collection named ' + collection + ' which doesn\'t exist. You must ' +
        ' first create the ' + collection + ' collection.';
    ...
    if (hop(attr, 'via') && (! child.attributes[attr.via])) {
      error = 'on mapping custom foreign keys: ' + collectionName + '->' + collection + '. Attribute: `' +
        attr.via + '` does not exist on: collection: `' + collection + '`';
      throw new Error(error);
    }
    ...
    if (hop(attr, 'via') && hop(child.attributes[attr.via], 'foreignKey'))
      return;
    ...
    if (! hop(attr, 'via')) {
      attr.via = attribute.key + '_' + collection;
    }

     // Build up an object that can be used to build a join table
    tableAttributes[collection] = {
...
JoinTables.prototype.buildTable = function(columns) {
  ...
  table.tables = [];
  _.keys(columns).forEach(function(columnName) {
    table.tables.push(columns[columnName]);
  });
  ...
  if (! table.connection) {
    var err = "A 'dominant' property was not supplied for the collections in a many-to-many relationship. " +
      "One side of the relationship between '";
    _.keys(columns).forEach(function(columnName) {
      err += columns[columnName].collection + "' and '";
    });
    err = err.slice(0, -6);
    err += " needs a 'dominant: true' flag set so a join table can be created on the correct connection.";
...
JoinTables.prototype.buildCollectionName = function(columns) {
  ...
  keys = _.sortBy(keys, function(a) {
    return a.collection;
  });
  var collectionName = '';
  keys.forEach(function(key) {
    collectionName += columns[key].collection + '_' + columns[key].attribute + '__';
  });
  collectionName = collectionName.slice(0, -2);
...
JoinTables.prototype.findDominantConnection = function(columns) {
  ...
  _.keys(columns).forEach(function(key) {
    ...
    var collection = self.collections[column.collection];
    console.log('find dominant', column);
    if (dominant) {
      return;
    }
    if (!connection) {
      connection = collection.connection;
    } else if (collection.connection !== connection) {
      // Require a dominant collection for models not in the same connection.
      required = true;
    }
    if (self.searchForAttribute(column.collection, 'dominant')) {
      dominant = collection.connection;
      return;
    }
  });
  
  console.log('found', dominant, required, connection);
  if (dominant) {
    return dominant;
  }
  if (!required) {
...
JoinTables.prototype.searchForAttribute = function(collectionName, attributeName, value) {
  ...
  Object.keys(collection.attributes).forEach(function(key) {
    ...
    if (! value && hop(properties, attributeName))
      matching = key;
    if (hop(properties, attributeName) && properties[attributeName] === value)
      matching = key;
  });
...
JoinTables.prototype.uniqueTables = function(tables) {
  ...
  tables.forEach(function(table) {
    ...
    Object.keys(self.tables).forEach(function(tableName) {
      ...
      if (currentTable.joinedAttributes.indexOf(table.joinedAttributes[0]) === - 1)
        return;
      if (currentTable.joinedAttributes.indexOf(table.joinedAttributes[1]) === - 1)
        return;

      add = false;
    });

    if (hop(self.tables, table.identity))
      return;
    if (add)
...
JoinTables.prototype.updateAttribute = function(collection, attributes) {
  ...
  for (var attribute in attributes) {
    ...
    if (! hop(joined.table.attributes, collection + '_' + attribute))
      continue;
...
JoinTables.prototype.findReference = function(parent, collection, via) {
  ...
  for (var attribute in attributes) {
    if (! hop(attributes[attribute], 'foreignKey'))
      continue;
    if (! hop(attributes[attribute], 'references'))
      continue;
    if (attributes[attribute].references !== parent)
      continue;
    if (via && via !== attribute)
      continue;
...
JoinTables.prototype.findJoinTable = function(parent, child, via) {
  ...
  for (var table in this.tables) {
    ...
    if (tables.indexOf(parent) < 0)
      continue;
    if (tables.indexOf(child) < 0)
      continue;

    var column = child + '_' + via;

    if (! hop(this.tables[table].attributes, column))
      continue;
